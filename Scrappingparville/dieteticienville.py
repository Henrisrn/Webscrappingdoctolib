...
Listedevilldecharlatan = [["marseille",1,11],["lyon",1,11],["paris",1,46]]
...
try:
    for nomville in Listedevilldecharlatan:
        for page in range(nomville[1],nomville[2]):
            ...
            driver.get("https://www.doctolib.fr/dieteticien/"+str(nomville[0])+"?page="+str(page))
            print("\n URL ENTREE : https://www.doctolib.fr/dieteticien/"+str(nomville[0])+"?page="+str(page)+"\n")
    ...
    full_df.to_csv("dieteticien.csv",quoting=csv.QUOTE_ALL,quotechar='"')
...
full_df.head()
full_df.to_csv("dieteticienville.csv",quoting=csv.QUOTE_ALL)
full_df.to_excel("dieteticienville.xlsx" )
